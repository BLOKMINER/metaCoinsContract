...
contract TokenVesting is  ReentrancyGuard {
    ...
    mapping(address => uint256) withdrawableAmount;
  ...
  mapping(address => mapping(uint256 => bool)) public isWithdrawer;
  ...
  event Withdrawn(uint256, uint256, address);
    ...
    uint256[] memory listOfWithdrawableAmount, address conditionContract) external nonReentrant{
        ...
        require(IERC20(token).balanceOf(msg.sender)>whiteListFeeAmount,"Insufficient balance");
        require(IERC20(token).allowance(msg.sender, address(this))> whiteListFeeAmount,"Contract has not been approved");
        IERC20(token).transferFrom(msg.sender, treasury, whiteListFeeAmount);
        ...
        for(uint256 i=0; i< totalWithdrawer;i++){
            isWithdrawer[listOfWithdrawer[i]][_lockId]=true;
            idToLock[_lockId].withdrawableAmount[listOfWithdrawer[i]] = listOfWithdrawableAmount[i];
        }
    ...
    function withdraw(uint256 _lockId, uint256 amount) external nonReentrant{
        require(idToLock[_lockId].endEmission < block.timestamp,"Unlock period is not over");
        require(isWithdrawer[msg.sender][_lockId]== true, "Not a withdrawer for this lock ID");
        require(amount <= idToLock[_lockId].withdrawableAmount[msg.sender],"Enter a valid amount" );
        idToLock[_lockId].withdrawableAmount[msg.sender] = idToLock[_lockId].withdrawableAmount[msg.sender]-amount;
        idToLock[_lockId].withdrawnAmount[msg.sender]= idToLock[_lockId].withdrawnAmount[msg.sender]+amount;
    ...
    function prematureUnlocking(uint256 _lockId) external nonReentrant{
        ...
        IUnlockCondition(idToLock[_lockId].condition).unlockTokens();
    }
